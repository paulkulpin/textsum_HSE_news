...
class ruT5SumDataset(torch.utils.data.Dataset):
    def __init__(
            self, 
            path, 
            tokenizer, 
...
def rut5_collate_batch(pad_id, batch):
    input_ids = []
    labels = []
    for sample in batch:
...
class ruT5Summarization(torch.nn.Module):
    ...
    def evaluate(
        ...
        for batch in tqdm(dataloader, total=len(dataloader), desc='Evaluating'):
            ...
            metrics = compute_metrics(
                (output_sequences.cpu(), batch['labels'].cpu()), 
                tokenizer, 
                rouge_metric, 
                bleu_metric)
