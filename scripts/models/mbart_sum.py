...
class MBARTSumDataset(torch.utils.data.Dataset):
    def __init__(self, path, tokenizer, max_input_length, max_target_length, document_field_name, summary_field_name):
        super().__init__()
...
def mbart_collate_batch(pad_id, batch):
    input_ids = []
    labels = []
    for sample in batch:
