...
def preprocess_function(row, ds_type, tokenizer, max_input_length, max_target_length, document_field_name, summary_field_name):
    ...
    if ds_type == 'train':
        input = dict()
        input['input_ids'] = model_inputs['input_ids'] + [tokenizer.sep_token_id] + labels["input_ids"]
        input['labels'] = model_inputs['input_ids'] + [tokenizer.sep_token_id] + labels["input_ids"]
    else:
        input = dict()
        input['input_ids'] = model_inputs['input_ids'] + [tokenizer.sep_token_id]
        input['labels'] = labels["input_ids"]
        
    return input


class ruGPTSumDataset(torch.utils.data.Dataset):
    def __init__(
            self, 
            path, 
            tokenizer, 
...
def rugpt_collate_batch(pad_id, batch):
    input_ids, labels = [], []
    for sample in batch:
...
def compute_metrics(eval_pred, tokenizer, rouge_metric, bleu_metric):
    ...
    for i in range(len(decoded_preds)):
        ...
        decoded_preds[i] = pred.split(tokenizer.eos_token)[0]
...
class ruGPTSummarization(torch.nn.Module):
