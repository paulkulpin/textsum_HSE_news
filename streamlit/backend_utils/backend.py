...
async def summarize(item: Item):
    ...
    if item.model_name == "ruT5-base":
        ...
        model.load_state_dict(torch.load('/Users/pavelkulpin/Downloads/rut5_base_sum_gazeta_state.pth')) #, map_location=device
        model.to(device)
        ...
        model = T5ForConditionalGeneration(config=config)
        model.load_state_dict(torch.load('/Users/pavelkulpin/Downloads/fred_model_state.pth'))
        model.to(device)
        ...
        model.load_state_dict(torch.load('/Users/pavelkulpin/Downloads/mbart_model_state.pth')) #.to(device)
        model.to(device)
