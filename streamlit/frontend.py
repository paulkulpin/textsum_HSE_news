...
with st.expander("Дополнительные параметры генерации"):
    st.markdown("Рекомендация к длине аннотации. При :green[length_penalty>0] будут поощряться более длинные тексты, при length_penalty < 0 - более короткие.")
    ...
    st.markdown("Ограничение числа наиболее вероятных кандидатов при генерации следующего токена. :green[top_k] указывает на число кандидатов, а :green[top_p] отсекает долю наименее вероятные варианты по статистическую вероятность.")
    ...
    num_beams = st.slider("num_beams:", min_value=1, max_value=10, value=4, step=1)
    st.divider()

    st.markdown("Контроль разнообразия генерации. Чем выше :green[temperature], тем стабильнее, но детерминированнее результат.")
...
if sum_button:
    ...
    if len(st.session_state['recent_summaries']) > 5:
        st.session_state['recent_summaries'] = st.session_state['recent_summaries'][:5]
        st.session_state['recent_params'] = st.session_state['recent_params'][:5]
